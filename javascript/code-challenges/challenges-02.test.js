...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(element => { 
    let newElemnt = Math.pow(element, 3);
    raisedToTheThirdArr.push(newElemnt)
  });
...
const addOne = (arr) => {
  ...
  let addOneArr = arr.map((val) => val +1)
  return addOneArr
}
...
const addQuestion = (arr) => {
  let addQuestionArr = arr.map(element => element + '?'
  )
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    raisedToTheThirdArr.push(Math.pow(2,arr[i]))
...
const mapTwoToThe = (arr) => {
  let mapTwoToTheArr = arr.map(element => Math.pow(2,element))
...
const charCode = (arr) => {
  let charCodeArr = arr.map((element, idx) => element.charCodeAt(idx))
  return charCodeArr
};
