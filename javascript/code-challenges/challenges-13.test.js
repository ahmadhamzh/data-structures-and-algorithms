...
const longestString = (arr) => {
    let lengthArr = arr.map(string => {
        return string.length
    })
    let longestLength = Math.max(...lengthArr);
    const findLongString = arr.find(string => {
        return string.length === longestLength
    })

    return arr.indexOf(findLongString)
};
...
const findHappiness = (arr) => {
  let happinessArr = arr.filter(string => string.includes(':)'));
  return happinessArr
};
...
const standardizePhoneNumbers = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
      ...
      for (let r = 0; r < arr[i].length; r++) {            
          if (num.includes(arr[i][r])) {
              arrOfNum.push(arr[i][r])    
              
          }            
      }
      arrOfAllNum.push(arrOfNum.join(''))   
