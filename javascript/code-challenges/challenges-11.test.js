...
function transformToLis(obj) {
  let keyValueArr = [];
  let keyObjectArr = Object.keys(obj)
  let valueObjectArr = Object.values(obj)
  for (let i = 0; i < keyObjectArr.length; i++) {
    keyValueArr.push(`<li>${keyObjectArr[i]}: ${valueObjectArr[i]}</li>`)
...
let findShortest = (data) => {
  let namesArr = [];
  data.map(hero => {
    namesArr.push(hero.height)
  })
  let shortest = Math.min(...namesArr)

  let heroName = data.find(hero => hero.height == shortest)
