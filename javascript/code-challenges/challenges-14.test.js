...
const toTitleCase = (arr) => {
  let firstA = arr.map(name => name.toUpperCase().charAt());
  let deletDirstLetter = arr.map(name => name.substring(1));
  let firstIsCappital = [];
  for (let i = 0; i < arr.length; i++) {
    let stringName = firstA[i]+deletDirstLetter[i]
    firstIsCappital.push(stringName)
    
  }
  return firstIsCappital
};
...
let sortBy = (property,arr) => {
  let sortedArr = [];
  if (property !== 'price') {
    let arrOfNames = arr.map(element => element.name)
    arrOfNames = arrOfNames.sort()
    for (let i = 0; i < arr.length; i++) {
      const found = arr.find(element => element.name == arrOfNames[i])
      sortedArr.push(found)
    }           
